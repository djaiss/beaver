...
<header class="sticky top-0 z-30 flex h-15 items-center gap-4 overflow-x-auto border-b border-hairline bg-sidebar px-4 lg:px-6">
    ...
    <span class="shrink-0 truncate text-sm font-semibold text-ink">{{ $catalog->name }}</span>
    ...
    <nav class="flex shrink-0 items-center gap-1">
        ...
        <a href="{{ route('sets.index', $catalog) }}" data-turbo="true" class="rounded-md px-3 py-1.5 text-[13px] font-medium text-muted transition-colors hover:text-ink">{{ __('Sets') }}</a>
        <a href="{{ route('statistics.index', $catalog) }}" data-turbo="true" class="rounded-md px-3 py-1.5 text-[13px] font-medium text-muted transition-colors hover:text-ink">{{ __('Statistics') }}</a>
        <a href="{{ route('categories.index', $catalog) }}" data-turbo="true" class="rounded-md px-3 py-1.5 text-[13px] font-medium text-muted transition-colors hover:text-ink">{{ __('Manage categories') }}</a>
    </nav>
