<div class="overflow-hidden rounded-xl border border-hairline">
    @foreach ($rows as $row)
        ...
        >
            ...
            @else
                <div class="flex size-11 shrink-0 items-center justify-center rounded-lg bg-card text-lg">{{ $catalog->emoji ?? '📦' }}</div>
            @endif
            <a href="{{ route('items.show', [$catalog, $row['id']]) }}" data-turbo="true" class="min-w-0 flex-1 truncate text-sm font-semibold text-ink transition-colors hover:text-muted">{{ $row['name'] }}</a>
            <div class="shrink-0">
