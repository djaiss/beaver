<div class="flex h-[calc(100vh-48px)] flex-col overflow-hidden bg-white rounded-bl-lg rounded-tl-lg">
  ...
  <form x-target="persons" action="{{ route('vault.person.search.create', [$vault->id]) }}" method="POST" class="shrink-0 border-b border-gray-200 p-3">
    ...
    <div class="relative">
      ...
      <x-input @input.debounce="$el.form.requestSubmit()" id="term" type="text" placeholder="{{ __('Search someone') }}" class="w-full border border-gray-300 bg-gray-100 py-1 pr-3 pl-8 text-sm focus:bg-white" autocomplete="off" :error="$errors->get('term')" />
    </div>
  </form>
