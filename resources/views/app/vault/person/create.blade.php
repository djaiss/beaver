        ...
        <x-form method="post" :action="route('vault.person.create', $vault->id)" class="space-y-4">
          <div
            ...
            x-data="{
              ...
              selectedKidsStatus: @js(__($kidsStatuses[old('kids_status', '')])),
            }">
            <div x-cloak x-show="showPrefix" x-transition class="relative">
              <x-input id="prefix" :label="__('Prefix')" :help="__('Prefix is a title or honorific that precedes a name, like Mr., Mrs., or Dr.')" :error="$errors->get('prefix')" :value="old('prefix')" x-ref="prefix" />
            </div>

            <div class="relative">
              <x-input id="first_name" :label="__('First name')" :error="$errors->get('first_name')" :value="old('first_name')" required autofocus />
            </div>

            <div class="relative">
              <x-input id="last_name" :label="__('Last name')" :error="$errors->get('last_name')" :value="old('last_name')" />
            </div>
            ...
            <div x-cloak x-show="showKids" x-transition class="rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-blue-900">
              @foreach ($kidsStatuses as $value => $label)
                <div class="flex items-center gap-x-3 p-3 not-last:border-b not-last:border-gray-200 dark:not-last:border-gray-700">
                  <input id="kids-status-{{ $value ?: 'unknown' }}" value="{{ $value }}" name="kids_status" type="radio" @checked(old('kids_status', '') === $value) x-on:click="selectedKidsStatus = @js(__($label))" class="relative size-4 appearance-none rounded-full border border-gray-300 bg-white before:absolute before:inset-1 before:rounded-full before:bg-white not-checked:before:hidden checked:border-indigo-600 checked:bg-indigo-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 forced-colors:appearance-auto forced-colors:before:hidden" />
                  <label for="kids-status-{{ $value ?: 'unknown' }}" class="block text-sm/6 font-medium text-gray-900 dark:text-gray-100">{{ __($label) }}</label>
                </div>
              @endforeach
            </div>

            <div x-cloak x-show="showMiddleName" x-transition class="relative">
              <x-input id="middle_name" :label="__('Middle name')" :error="$errors->get('middle_name')" :value="old('middle_name')" x-ref="middleName" />
            </div>

            <div x-cloak x-show="showNickname" x-transition class="relative">
              <x-input id="nickname" :label="__('Nickname')" :error="$errors->get('nickname')" :value="old('nickname')" x-ref="nickname" />
            </div>

            <div x-cloak x-show="showMaidenName" x-transition class="relative">
              <x-input id="maiden_name" :label="__('Maiden name')" :help="__('Maiden name is the name a person used before they were married.')" :error="$errors->get('maiden_name')" :value="old('maiden_name')" x-ref="maidenName" />
            </div>

            <div x-cloak x-show="showSuffix" x-transition class="relative">
              <x-input id="suffix" :label="__('Suffix')" :help="__('Suffix is a term that follows a name, like Jr., Sr., or III.')" :error="$errors->get('suffix')" :value="old('suffix')" x-ref="suffix" />
            </div>

            <div x-cloak x-show="showGender" x-transition class="relative">
              <x-select id="gender_id" :label="__('Gender')" :options="$genders" :selected="old('gender_id')" :error="$errors->get('gender_id')" />
            </div>

            <div class="flex flex-wrap text-xs">
              @foreach ([['state' => 'showPrefix', 'ref' => 'prefix', 'label' => '+ prefix'], ['state' => 'showMiddleName', 'ref' => 'middleName', 'label' => '+ middle name'], ['state' => 'showSuffix', 'ref' => 'suffix', 'label' => '+ suffix'], ['state' => 'showNickname', 'ref' => 'nickname', 'label' => '+ nickname'], ['state' => 'showMaidenName', 'ref' => 'maidenName', 'label' => '+ maiden name']] as $field)
