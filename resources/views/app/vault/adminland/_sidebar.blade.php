<aside class="text-sm flex-col border-b border-gray-200 bg-white px-4 py-4 sm:flex sm:rounded-bl-lg sm:border-r sm:border-b-0 dark:border-gray-700 dark:bg-gray-900">
  <nav class="flex flex-col gap-1">
    <!-- section: account -->
    <p class="mb-1 text-sm font-medium text-gray-500">{{ __('Settings') }}</p>
    <a data-turbo="true" href="{{ route('vault.adminland.index', ['vaultId' => $vault->id]) }}" class="{{ request()->routeIs('vault.adminland.index') ? 'bg-gray-100 font-medium text-gray-900 dark:bg-gray-800 dark:text-gray-100' : 'text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800' }} flex items-center gap-2 rounded-lg px-2 py-1">
      <x-phosphor-vault class="h-4 w-4 {{ request()->routeIs('vault.adminland.index') ? 'text-emerald-700' : 'text-gray-500' }}" />
      {{ __('Vault') }}
    </a>
