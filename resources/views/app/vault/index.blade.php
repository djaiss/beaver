<?php
/**
 * @var \App\ViewModels\Vault\VaultIndexViewModel $view
 */
?>
    ...
    <div class="mx-auto w-full max-w-4xl items-start justify-center">
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-x-2">
          <x-button.secondary href="{{ $view->url()->join }}" turbo="true">{{ __('app/vault.index.join') }}</x-button.secondary>

          <x-button href="{{ $view->url()->create }}" turbo="true">
            <x-slot:icon>
        ...
        @forelse ($view->vaults() as $vault)
          <div class="rounded-0 flex items-center justify-between border-b border-gray-200 first:rounded-t-lg last:rounded-b-lg last:border-b-0 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">
            <div class="flex items-center">
              ...
              <x-link href="{{ $vault->url }}">{{ $vault->name }}</x-link>
            </div>
          </div>
        @empty
          <x-empty-state>
