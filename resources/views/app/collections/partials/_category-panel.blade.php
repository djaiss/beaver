...
@php
    ...
    $collectionItems = $collectionTotals['items'];
    $collectionValue = $collectionTotals['value'];
        ...
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="flex items-center gap-1.5 rounded-full border border-hairline bg-canvas px-2.5 py-0.5 text-xs font-medium text-muted">
                    ...
                    {{ __('Category in :collection', ['collection' => $collection->name]) }}
                </span>
            </div>
    ...
    <div>
        ...
        <div class="flex h-3 gap-0.5 overflow-hidden rounded-full">
            @foreach ($categoryBreakdown as $row)
                <a
                    href="{{ route('categories.show', [$collection, $row['id']]) }}"
                    data-turbo="true"
                    title="{{ $row['name'] }}"
                    class="h-full"
