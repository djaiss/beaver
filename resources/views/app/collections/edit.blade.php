<x-app-layout :collection="$collection">
  <x-slot:title>
    {{ __('Edit :name', ['name' => $collection->name]) }}
  </x-slot>
  ...
  <div class="px-6 py-8 lg:px-12 lg:py-10">
    <div class="mx-auto w-full max-w-2xl space-y-8">
      ...
      <x-form method="put" :action="route('collections.update', $collection->id)" data-turbo="true" data-test="edit-collection-form">
        <div x-data="{
          emoji: @js(old('emoji', $collection->emoji ?? $emojiOptions[0])),
          visibility: @js(old('visibility', $collection->visibility->value)),
          ...
          {{-- Name --}}
          <x-input id="name" :label="__('Name')" placeholder="{{ __('e.g. Marvel Comics 1990s') }}" :value="old('name', $collection->name)" :error="$errors->get('name')" required autofocus data-test="collection-name-input" />

          {{-- Description --}}
          <x-textarea id="description" :label="__('Description')" placeholder="{{ __('What\'s in this collection?') }}" rows="3" :value="old('description', $collection->description)" :error="$errors->get('description')" />
          ...
          <div class="flex items-center justify-end gap-3 pt-2">
            <x-button.secondary href="{{ route('collections.show', $collection->id) }}" turbo="true">
              {{ __('Cancel') }}
            </x-button.secondary>
