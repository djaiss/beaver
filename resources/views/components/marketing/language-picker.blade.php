...
<div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false" class="relative inline-block">
  <button
    ...
    :aria-expanded="open"
    aria-haspopup="true"
    class="flex items-center gap-x-2 rounded-md border border-[#242424] px-2.5 py-2 text-[13px] font-medium text-[#a1a1aa] transition-colors hover:bg-[#1a1a1a] hover:text-white"
  >
  ...
  <div
    x-show="open"
    x-cloak
    class="absolute bottom-11 left-0 z-50 w-52 rounded-xl border border-[#242424] bg-[#161616] p-1.5 shadow-xl"
  >
    @foreach ($links as $link)
      <a
        href="{{ $link['url'] }}"
        lang="{{ str_replace('_', '-', $link['locale']) }}"
        class="flex items-center gap-x-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium transition-colors hover:bg-[#1f1f1f] {{ $link['current'] ? 'text-white' : 'text-[#a1a1aa]' }}"
      >
