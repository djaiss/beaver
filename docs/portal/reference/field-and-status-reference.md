...
Every option set you meet in a KolleK form, in one scannable place. Each group links to the guide that uses it. For definitions of the terms themselves, see the [glossary](glossary.md).

## Copy statuses

Set on each copy you record. Used in [Track the copies you own](../core-features/track-the-copies-you-own.md).
...
Set on each transaction. Used in [Record what you paid and what it is worth](../copy-history/record-payments-and-value.md). Types marked acquiring bring a copy into your hands, and the earliest acquiring transaction provides the copy's acquisition date.
...
Set on each valuation. Used in [Record what you paid and what it is worth](../copy-history/record-payments-and-value.md).
...
Set on each insurance record. Used in [Insure a copy](../copy-history/insure-a-copy.md). The coverage type on an insurance record is free text, so it has no fixed option list.
...
Set on each loan. Used in [Lend and borrow copies](../copy-history/lend-and-borrow-copies.md).
...
Set on each maintenance record. Used in [Record maintenance and repairs](../copy-history/record-maintenance-and-repairs.md).
...
Set on each provenance event. Used in [Trace a copy's provenance](../copy-history/trace-provenance.md).
...
Set on each document. Used in [Attach documents to a copy](../copy-history/attach-documents.md).
...
Chosen when defining a custom field on a collection type. Used in [Set up collection types and custom fields](../organizing/set-up-collection-types-and-custom-fields.md).
...
Set on each collection. Used in [Share a collection](../collaboration/share-a-collection.md). The setting is recorded today and enforced once sharing arrives; see [Feature status and roadmap](../troubleshooting/feature-status.md).
...
- What the terms mean: [Glossary](glossary.md).
- The records these options live on: [A copy's history explained](../core-concepts/copy-history.md).
