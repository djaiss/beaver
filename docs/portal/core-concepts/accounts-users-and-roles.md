...
An **account** is a private workspace. Every collection, item, copy, type, tag, and location lives inside exactly one account. Nothing leaks between accounts, and nobody outside yours can see in unless you deliberately [share a collection](visibility-and-sharing.md).
...
Reading is open to every member, including viewers. Writing needs editor or owner. Administering the account needs owner. The [roles in practice](../collaboration/roles-in-practice.md) page maps this to concrete tasks if you want the full table.
