...
:::note
Types describe items, not copies. A field that varies per physical piece you own, such as condition or a serial number, belongs on the copy instead. See [Items versus copies](items-and-copies.md).
:::

## Where to next

- Build or adjust a type step by step in [Set up collection types and custom fields](../organizing/set-up-collection-types-and-custom-fields.md).
- Share a type definition with someone in [Import and export a collection type](../organizing/import-and-export-a-collection-type.md).
- See fields in action in [Add and edit items](../core-features/add-and-edit-items.md).
