...
If you have not yet decided between self hosting and a hosted instance, start with [Cloud version versus self hosting](../getting-started/cloud-vs-self-hosting.md).
...
This is worth reading about properly before you install. [The application key and encryption](application-key-and-encryption.md) explains what the key protects, how to store it, and the one safe way to rotate it deliberately.
...
1. [Install with Docker](install-with-docker.md). From nothing to a running instance.
2. [Configure your instance](configure-your-instance.md). The environment variables you will actually touch.
3. [Set up email delivery](set-up-email-delivery.md). Make invitations and magic links actually send.
4. [The application key and encryption](application-key-and-encryption.md). The most important operational rule.
5. [Upgrade your instance](upgrade-your-instance.md). Move to a new version safely.
6. [Back up and restore your instance](back-up-and-restore.md). Protect the data.
7. [Scheduled maintenance jobs](scheduled-jobs.md). What the app does on its own every night.
8. [Grant instance administrator access](grant-instance-administrator-access.md). Bootstrap the server wide administrator.
9. [The instance administration panel](instance-administration-panel.md). What that administrator can see and do.
10. [Administer with the command line](cli-commands.md). The artisan commands an operator needs.
11. [Add a language](add-a-language.md). How the interface is translated.

## Where to next

- Ready to install? Go to [Install with Docker](install-with-docker.md).
- Prefer a guided end to end walkthrough? Follow the [self hosting tutorial](../tutorials/self-host-with-docker.md).
