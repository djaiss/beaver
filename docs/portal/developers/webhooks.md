...
On a self hosted instance, deliveries run on the queue worker, so the queue role must be running. See [Install with Docker](../self-hosting/install-with-docker.md).

## Where to next

- Check what is live and what is pending on the [feature status page](../troubleshooting/feature-status.md).
- Build against the API in the meantime, starting with [Authenticate with the API](authenticate-with-the-api.md).
