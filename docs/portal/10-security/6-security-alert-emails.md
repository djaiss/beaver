...
**If it was not you**, someone is trying your address. One failed attempt is usually just noise, but repeated alerts mean your email is being targeted. Make sure your password is unique to KolleK, and turn on @doc(security.twoFactorAuth, "two factor authentication") so a guessed password is not enough.
...
**If it was not you**, someone has your password. @doc(auth.resetPassword, "Change your password") immediately, and review your account for anything unexpected.
...
**When it arrives:** an @doc(apiKeys.manage, "API key") was created or revoked on your user.
...
Two other emails arrive only because someone requested them, so they are not alerts in themselves: the @doc(auth.magicLinks, "magic link") email, and the password reset email. If you receive one you did not request, someone entered your address in that form. Neither can be used without access to your inbox, but repeated unrequested emails are another sign your address is being poked at.

## If something genuinely looks wrong

1. @doc(auth.resetPassword, "Change your password").
2. Turn on @doc(security.twoFactorAuth, "two factor authentication") if it is off.
3. Review your @doc(apiKeys.manage, "API keys") and revoke anything you do not recognize.
4. Check @doc(activity.logAndSentEmails, "your personal activity log") for actions you did not take.

## Where to next

- See everything KolleK has ever sent you, with delivery status: @doc(activity.logAndSentEmails, "Your personal activity log and sent emails").
- The full catalogue of every email KolleK can send: @doc(reference.emailsSent).
