...
You can change your password at any time, and recover access if you forget it. See @doc(auth.resetPassword).
...
Set it up in @doc(security.twoFactorAuth), and make sure you understand @doc(security.recoveryCodes, "recovery codes") before you rely on it.

## Recovery codes

When you enable two factor authentication, KolleK gives you eight recovery codes. Each one can be used once, in place of an authenticator code, to get back in if you lose your phone. Store them somewhere safe. @doc(security.recoveryCodes) explains how.

## Magic links

A passwordless way to sign in. KolleK emails you a link that signs you in directly, valid for five minutes. Convenient, with one trade off worth understanding: a magic link does not ask for a two factor code, because access to your inbox already acts as the second factor. @doc(auth.magicLinks) covers when to use them.

## API keys

If you use the KolleK API, you authenticate with personal API keys. They are created and revoked from your profile, and KolleK emails you whenever one is created or deleted, so a key you did not make never goes unnoticed. See @doc(apiKeys.manage).

## Alert emails

KolleK watches for events worth telling you about: a failed sign in attempt, a sign in from a new device, a change in your IP address, an API key created or deleted. When one happens, you get an email. @doc(security.alertEmails) explains what each alert means and what to do about it.

## A sensible setup

If you only do two things, do these:

1. Turn on @doc(security.twoFactorAuth, "two factor authentication").
2. Save your @doc(security.recoveryCodes, "recovery codes") somewhere that is not your phone.
