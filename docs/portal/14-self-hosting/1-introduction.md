...
If you have not yet decided between self hosting and a hosted instance, start with @doc(kollek.hostingOptions).
...
This is worth reading about properly before you install. @doc(selfHosting.applicationKeyAndEncryption) explains what the key protects, how to store it, and the one safe way to rotate it deliberately.
...
1. @doc(selfHosting.installDocker). From nothing to a running instance.
2. @doc(selfHosting.configure). The environment variables you will actually touch.
3. @doc(selfHosting.setupEmailDelivery). Make invitations and magic links actually send.
4. @doc(selfHosting.applicationKeyAndEncryption). The most important operational rule.
5. @doc(selfHosting.upgrade). Move to a new version safely.
6. @doc(selfHosting.backupAndRestore). Protect the data.
7. @doc(selfHosting.scheduledJobs). What the app does on its own every night.
8. @doc(instanceAdmin.grantAccess). Bootstrap the server wide administrator.
9. @doc(instanceAdmin.panel). What that administrator can see and do.
10. @doc(selfHosting.cliCommands). The artisan commands an operator needs.
11. @doc(selfHosting.addLanguage). How the interface is translated.

## Where to next

- Ready to install? Go to @doc(selfHosting.installDocker).
- Prefer a guided end to end walkthrough? Follow the @doc(tutorials.selfHostWithDocker, "self hosting tutorial").
