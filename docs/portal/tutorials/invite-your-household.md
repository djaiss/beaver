...
- Read [Accounts, users, and roles](../core-concepts/accounts-users-and-roles.md) if you have not. The one line version: viewers read, editors change catalogue content, owners also manage the account.
...
If the link says an account already exists for that email, that person cannot join through this invitation. That situation and other invitation hiccups are covered in [Troubleshooting sign in](../troubleshooting/troubleshoot-sign-in.md).

## Step 4: Set each collection's visibility

Roles control the people inside the account. [Visibility](../core-concepts/visibility-and-sharing.md) records who each collection is meant for, from just you to anyone with a link.
...
The full walkthrough, including reverting, is in [Share a collection](../collaboration/share-a-collection.md).
...
- The full reference of who can do what is in [Understanding the three roles in practice](../collaboration/roles-in-practice.md).
- Manage the account itself, name, currency, and more, in [Account settings](../account-and-profile/account-settings.md).
- Running the instance for your club yourself? See [Self host KolleK with Docker](self-host-with-docker.md).
