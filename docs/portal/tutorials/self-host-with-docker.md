...
It also helps to skim the [self hosting overview](../self-hosting/README.md) first, because it introduces the one rule this tutorial will insist on: the application key is set once and never changed.
...
The `.env` file is your instance's configuration. Everything an operator routinely touches lives in it, and the [configuration guide](../self-hosting/configure-your-instance.md) walks through it group by group. For a first boot, only the next two steps are mandatory.
...
The full story, including how deliberate key rotation is supported, is in [The application key and encryption](../self-hosting/application-key-and-encryption.md).
...
Go to the registration page and sign up. This works exactly as it does for any user, the walkthrough is in [Create your account](../getting-started/create-your-account.md), and it makes you the owner of the instance's first account.
...
- Walk through every setting you skipped in [Configure your instance](../self-hosting/configure-your-instance.md).
- Set up [backups](../self-hosting/back-up-and-restore.md) before the catalogue grows precious.
- When a new version ships, follow [Upgrade your instance](../self-hosting/upgrade-your-instance.md).
