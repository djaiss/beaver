...
- Finish [Catalogue your first collection end to end](catalogue-your-first-collection.md) first. This tutorial assumes the core loop is second nature.
- Read [A copy's history explained](../core-concepts/copy-history.md). It is the map for everything below.
...
With a professional appraisal in hand, insurance is the obvious next step. Priya adds an [insurance record](../copy-history/insure-a-copy.md): the **provider**, the **insured value**, the **policy number**, the **coverage type**, the **deductible**, the **start and end dates**, whether it is a **scheduled item** on the policy, and the insurer's contact details. She leaves the status **Active**.
...
Records are claims. Documents are proof. Priya scans two pieces of paper and [attaches them](../copy-history/attach-documents.md) where they belong:
...
A local horology society asks to display the watch for a month. Custody is exactly what [loans](../copy-history/lend-and-borrow-copies.md) track.
...
Before the watch went on display, Priya had it serviced. She adds a [maintenance record](../copy-history/record-maintenance-and-repairs.md) of type **Servicing**: a title, the watchmaker who performed it, the date, the cost, the condition before and after, and a **next due date** five years out so the app can surface the next service when it approaches. Since a full service on a vintage movement is significant, she chooses to include it in the copy's provenance.

## Step 8: Build the provenance narrative

Finally, the ownership story. Priya knows the watch's past from the auction catalogue, and she records it as [provenance events](../copy-history/trace-provenance.md), oldest first:
...
- Every record type used here has its own detailed guide in the [copy history section](../copy-history/README.md).
- See how these records feed the numbers in [Understand your collection statistics](../insights/collection-statistics.md).
- Sharing the collection with others? [Invite your household or club](invite-your-household.md).
