...
An **account** is a private workspace. Every collection, item, copy, type, tag, and location lives inside exactly one account. Nothing leaks between accounts, and nobody outside yours can see in unless you deliberately @doc(sharing.overview, "share a collection").
...
Reading is open to every member, including viewers. Writing needs editor or owner. Administering the account needs owner. The @doc(collaboration.rolesInPractice, "roles in practice") page maps this to concrete tasks if you want the full table.
...
If you ever hear about an **instance administrator**, that is something else entirely. It is a server wide flag for whoever operates the KolleK installation itself. It grants nothing extra inside that person's own account, and it has nothing to do with viewer, editor, or owner. It is covered in @doc(instanceAdmin.panel, "the instance administration panel") for operators.

## Where to next

- Bring someone in with @doc(collaboration.invitePeople).
- Change what a member can do in @doc(collaboration.manageMembersAndRoles).
- Continue the concepts with @doc(collections.overview).
