...
it('puts a tag on an item', function () {
    ...
    $response = $this->actingAs($user)
        ->post("/collections/{$collection->id}/items/{$item->id}/tags", ['name' => 'Signed']);
...
it('creates a tag the account does not have yet', function () {
    ...
    $this->actingAs($user)
        ->post("/collections/{$collection->id}/items/{$item->id}/tags", ['name' => 'First print'])
...
it('requires a name to put a tag on an item', function () {
    ...
    $this->actingAs($user)
        ->post("/collections/{$collection->id}/items/{$item->id}/tags", ['name' => ''])
...
it('takes a tag off an item', function () {
    ...
    $response = $this->actingAs($user)
        ->delete("/collections/{$collection->id}/items/{$item->id}/tags/{$tag->id}");
...
it('does not tag an item belonging to another account', function () {
    ...
    $this->actingAs($user)
        ->post("/collections/{$foreignCollection->id}/items/{$foreignItem->id}/tags", ['name' => 'Signed'])
        ->assertNotFound();
});
...
it('does not take off a tag belonging to another account', function () {
    ...
    $this->actingAs($user)
        ->delete("/collections/{$collection->id}/items/{$item->id}/tags/{$foreignTag->id}")
        ->assertNotFound();
});
