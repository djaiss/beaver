...
it('moves a copy to a location', function () {
    ...
    $response = $this->actingAs($user)->post(route('locationHistory.create', [$collection, $item, $copy]), [
        'location_id' => $shelf->id,
        'moved_at' => '2024-01-01',
        'reason' => 'Catalogued',
    ]);

    $response->assertRedirect(route('items.history.show', [$collection, $item, $copy, 'locations']));
...
it('requires a destination and a date', function () {
    ...
    $this->actingAs($user)->post(route('locationHistory.create', [$collection, $item, $copy]), [])
...
it('does not move a copy of another account', function () {
    ...
    $this->actingAs($user)->post(route('locationHistory.create', [$otherCollection, $otherItem, $otherCopy]), [
...
it('forbids a viewer from moving a copy', function () {
    ...
    $this->actingAs($viewer)->post(route('locationHistory.create', [$collection, $item, $copy]), [
        'location_id' => $location->id,
        'moved_at' => '2024-01-01',
    ])->assertNotFound();
});
...
it('corrects a record', function () {
    ...
    $response = $this->actingAs($user)->put(route('locationHistory.update', [$collection, $item, $copy, $record]), [
        'location_id' => $shelf->id,
        'moved_at' => '2024-02-15',
...
it('deletes a record', function () {
    ...
    $response = $this->actingAs($user)->delete(route('locationHistory.destroy', [$collection, $item, $copy, $record]));
...
it('forbids a viewer from deleting a record', function () {
    ...
    $this->actingAs($viewer)->delete(route('locationHistory.destroy', [$collection, $item, $copy, $record]))->assertNotFound();
});
...
it('shows the location history of a copy on the history tab', function () {
    ...
    $this->actingAs($user)->get(route('items.history.show', [$collection, $item, $copy, 'locations']))
        ->assertOk()
...
it('does not render the move form for a viewer', function () {
    ...
    $this->actingAs($viewer)->get(route('items.history.show', [$collection, $item, $copy, 'locations']))
        ->assertOk()
