...
it('does not list another accounts types', function () {
    $user = $this->createUser();
    CatalogType::factory()->create(['name' => 'Foreign type']);
...
it('allows an editor to list types', function () {
    ...
    CatalogType::factory()->create(['account_id' => $account->id, 'name' => 'Comics']);
...
it('shows the edit page', function () {
    $user = $this->createUser();
    $type = CatalogType::factory()->create(['account_id' => $user->account_id, 'name' => 'Vinyl Records']);
...
it('links to each collection using the type', function () {
    $user = $this->createUser();
    $type = CatalogType::factory()->create(['account_id' => $user->account_id, 'name' => 'Vinyl Records']);
    $linked = Catalog::factory()->create(['account_id' => $user->account_id, 'name' => 'Central Perk Vinyl']);
    $linked->catalogTypes()->attach($type->id);
    $unlinked = Catalog::factory()->create(['account_id' => $user->account_id, 'name' => 'Joeys Baywatch Tapes']);
...
it('tells the user when no collection uses the type', function () {
    ...
    Catalog::factory()->create(['account_id' => $user->account_id, 'name' => 'Joeys Baywatch Tapes']);
...
it('does not offer the collections of another account', function () {
    $user = $this->createUser();
    $type = CatalogType::factory()->create(['account_id' => $user->account_id]);
    Catalog::factory()->create(['name' => 'Someone Elses Catalog']);

    $this->actingAs($user)->get('/settings/types/'.$type->id.'/edit')
        ->assertOk()
        ->assertDontSee('Someone Elses Catalog');
});
