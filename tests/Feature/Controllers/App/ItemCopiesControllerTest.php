...
it('shows the copies of an item with their condition and location', function () {
    ...
    $response = $this->actingAs($user)->get(route('items.copies.index', [$catalog, $item]));
...
it('calls out the quantity only when the copy stands for more than one instance', function () {
    ...
    $catalog = Catalog::factory()->create(['account_id' => $user->account_id]);
    $item = Item::factory()->create(['catalog_id' => $catalog->id]);
    $single = Item::factory()->create(['catalog_id' => $catalog->id]);
    ...
    $this->actingAs($user)->get(route('items.copies.index', [$catalog, $single]))
        ->assertOk()
...
it('shows the note of a copy, and says so when there is none', function () {
    ...
    $catalog = Catalog::factory()->create(['account_id' => $user->account_id]);
    $item = Item::factory()->create(['catalog_id' => $catalog->id]);
    $noted = Item::factory()->create(['catalog_id' => $catalog->id]);
    Copy::factory()->create(['item_id' => $noted->id, 'note' => 'Signed by Gunther.']);
    Copy::factory()->create(['item_id' => $item->id, 'note' => null]);

    $this->actingAs($user)->get(route('items.copies.index', [$catalog, $noted]))
        ->assertOk()
...
it('links a copy to the history tab', function () {
    ...
    $this->actingAs($user)->get(route('items.copies.index', [$catalog, $item]))
        ...
        ->assertSee(route('items.history.index', [$catalog, $item]), false);
});
...
it('lets a viewer read the copies of an item', function () {
    ...
    $this->actingAs($viewer)->get(route('items.copies.index', [$catalog, $item]))->assertOk();
});
...
it('does not show the copies of an item that belongs to a different collection', function () {
    ...
    $this->actingAs($user)->get(route('items.copies.index', [$catalog, $item]))->assertNotFound();
});
