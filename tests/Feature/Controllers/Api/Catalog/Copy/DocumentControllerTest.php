...
it('does not list the documents of a copy from another account', function () {
    ...
    Document::factory()->for($copy, 'documentable')->create(['account_id' => $copy->item->catalog->account_id]);
...
it('does not show a document from another account', function () {
    $user = $this->createUser();
    $copy = copyForApiDocuments($this->createAccount()->id);
    $document = Document::factory()->for($copy, 'documentable')->create(['account_id' => $copy->item->catalog->account_id]);
