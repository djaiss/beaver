...
it('shows the statistics of a collection', function () {
    ...
    $this->json('GET', '/api/collections/'.$catalog->id.'/statistics')
        ->assertOk()
        ...
        ->assertJsonPath('data.links.self', route('api.collections.statistics', $catalog->id));
});
...
it('shows the sets completion of a collection that has targeted sets', function () {
    ...
    $set = Set::factory()->create(['catalog_id' => $catalog->id, 'name' => 'Central Perk', 'target_count' => 4]);
    Item::factory()->create(['catalog_id' => $catalog->id, 'set_id' => $set->id, 'name' => 'Rachel']);
    Item::factory()->create(['catalog_id' => $catalog->id, 'set_id' => $set->id, 'name' => 'Monica']);
