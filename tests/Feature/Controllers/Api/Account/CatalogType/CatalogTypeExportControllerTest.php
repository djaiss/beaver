...
it('links back to itself and to the collection type', function () {
    ...
    $this->json('GET', '/api/collection-types/'.$type->id.'/export')
        ->assertOk()
        ->assertJsonPath('data.links.self', route('api.catalogTypes.export.show', $type->id))
        ->assertJsonPath('data.links.collection_type', route('api.catalogTypes.show', $type->id));
});

it('exports a type that has no group and no field', function () {
    $user = $this->createUser();
    $type = CatalogType::factory()->create(['account_id' => $user->account_id, 'name' => 'Vinyl']);
...
it('does not export a type of another account', function () {
    $user = $this->createUser();
    $type = CatalogType::factory()->create(['name' => 'Foreign type']);
...
it('rejects an unauthenticated request', function () {
    $type = CatalogType::factory()->create(['name' => 'Comics']);
