...
it('resolves a collection of the account and shares it with the views', function () {
    ...
    $this->actingAs($user)
        ->get('/collections/'.$catalog->id.'/categories')
        ->assertOk()
...
it('does not let laravel resolve the collection before the middleware does', function () {
    ...
    $stranger = $this->createUser();
    $catalog = Catalog::factory()->create(['account_id' => $stranger->account_id]);
    $category = $catalog->categories()->create(['name' => 'Central Perk']);

    $this->actingAs($user)
        ->put('/collections/'.$catalog->id.'/categories/'.$category->id, ['name' => 'Renamed'])
        ->assertNotFound();
