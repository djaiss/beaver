...
/**
 * A well formed document, in the exact shape ExportCatalogType hands out.
...
it('imports a type with its groups, fields and options', function (): void {
    ...
    expect($type)->toBeInstanceOf(CatalogType::class);
...
it('does not let a viewer import a type', function (): void {
    ...
    new ImportCatalogType(user: $viewer, account: $account, json: schemaJson())->execute();
});
...
it('does not let someone import into an account they do not belong to', function (): void {
    ...
    new ImportCatalogType(user: $stranger, account: $account, json: schemaJson())->execute();
});
...
it('rejects a document it cannot trust and writes nothing', function (string $json): void {
    ...
    expect(fn (): CatalogType => new ImportCatalogType(user: $owner, account: $account, json: $json)->execute())
        ->toThrow(ValidationException::class);

    expect($account->catalogTypes()->count())->toBe(0);
...
it('reports every problem in the document at once', function (): void {
    ...
    try {
        new ImportCatalogType(user: $owner, account: $account, json: $json)->execute();
...
it('imports a document exported by the export action', function (): void {
    ...
    $source = new ImportCatalogType(user: $owner, account: $account, json: schemaJson())->execute();

    $exported = new ExportCatalogType(user: $owner, catalogType: $source)->execute();

    $imported = new ImportCatalogType(
        user: $owner,
        account: $account,
    ...
    expect(new ExportCatalogType(user: $owner, catalogType: $imported)->execute())->toBe($exported);
});
