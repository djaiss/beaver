<?php

declare(strict_types=1);

return [
    'add_field' => [
        'gender' => '+ Geschlecht',
        'maiden_name' => '+ Geburtsname',
        'middle_name' => '+ zweiter Vorname',
        'nickname' => '+ Spitzname',
        'prefix' => '+ Präfix',
        'suffix' => '+ Suffix',
    ],
    'fields' => [
        'first_name' => 'Vorname',
        'gender' => 'Geschlecht',
        'kids_status' => 'Kinderstatus',
        'last_name' => 'Nachname',
        'maiden_name' => 'Geburtsname',
        'middle_name' => 'Zweiter Vorname',
        'nickname' => 'Spitzname',
        'prefix' => 'Präfix',
        'suffix' => 'Suffix',
    ],
    'help' => [
        'maiden_name' => 'Der Geburtsname ist der Name, den eine Person vor der Heirat verwendet hat.',
        'prefix' => 'Ein Präfix ist ein Titel oder eine Anrede vor einem Namen, wie Herr, Frau oder Dr.',
        'suffix' => 'Ein Suffix ist ein Zusatz nach einem Namen, wie Jr., Sr. oder III.',
    ],
    'new' => [
        'created' => 'Person wurde erfolgreich erstellt',
        'description' => 'Füge die derzeit bekannten Angaben hinzu. Optionale Informationen können später ergänzt werden.',
        'title' => 'Person hinzufügen',
    ],
    'options' => [
        'has_kids' => 'Hat Kinder',
        'maybe_kids' => 'Hat möglicherweise Kinder',
        'no_kids' => 'Hat keine Kinder',
        'unknown' => 'Unbekannt',
    ],
];
