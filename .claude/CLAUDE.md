...
- `app/Services`: everything else, which is three kinds. The read side both the web app and the JSON API ask questions of the database through (`AccountSearch`, `AccountDashboard`, `CatalogStatistics`, `BuildCopyHistory`, `Trash`, `LoanDashboard`, `GettingStarted`). The content layer that reads and transforms what sits on disk (`DocumentationPortal`, `DocumentationParser`, `HelpSnippets`, `ApiDocumentation`), which stays a service even when a view is its only caller today: `DocumentationPortal` was view only until the locale middleware needed it. And the plumbing models and actions reach for (`BlindIndex`, hashing search tokens as a record saves).
- The line between the two: a view model that renders JSON is a lie, and so is one that runs a query. If an API controller, an action, a model, a job or middleware needs the class, or if it touches the database or the filesystem, it is a service.
