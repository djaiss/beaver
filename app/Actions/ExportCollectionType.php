...
class ExportCollectionType
{
    ...
    {
        $groups = $this->collectionType->customFieldGroups()
            ->with(['customFields' => fn ($query) => $query->orderBy('position')->orderBy('id')])
            ->orderBy('position')
            ->orderBy('id')
            ->get();

        $standaloneFields = $this->collectionType->ungroupedCustomFields()
            ->orderBy('position')
            ->orderBy('id')
            ->get();
        ...
        return [
            ...
            'type' => [
                ...
                'color' => $this->collectionType->color,
                'groups' => $groups
