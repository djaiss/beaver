...
class PopulateAccount implements ShouldQueue
{
        ...
        DB::transaction(function (): void {
            ...
            $this->account->itemConditions()->createMany(
                collect($this->defaultConditions())
                    ->map(fn (string $name, int $index): array => ['name' => $name, 'position' => $index + 1])
                    ->all()
            );
        });
    }
    ...
    /**
     * The fields listed under "fields" stay ungrouped and render first, then
     * each group renders under its own heading.
     *
     * @param  array{name: string, color: string, fields: list<array{name: string, field_type: FieldTypeEnum, options?: list<string>}>, groups?: list<array{name: string, fields: list<array{name: string, field_type: FieldTypeEnum, options?: list<string>}>}>}  $type
     */
    ...
    /**
     * A position orders a field within its group, or within the type when the
     * field is ungrouped, so each list restarts at 1.
     *
     * @param  list<array{name: string, field_type: FieldTypeEnum, options?: list<string>}>  $fields
     * @return list<array{name: string, field_type: FieldTypeEnum, options: list<string>|null, position: int}>
     */
    ...
    /**
     * @return list<array{name: string, color: string, fields: list<array{name: string, field_type: FieldTypeEnum, options?: list<string>}>, groups?: list<array{name: string, fields: list<array{name: string, field_type: FieldTypeEnum, options?: list<string>}>}>}>
     */
    private function defaultTypes(): array
    {
        return [
            ...
            [
                // Bottle Size sits outside of any group, so a Wine type shows
                // both a standalone field and a group from the start.
                'name' => 'Wine',
                'color' => '#991B1B',
                'fields' => [
