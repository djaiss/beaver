...
class CatalogTypeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $types = $account->catalogTypes()
            ->with('customFields')
            ->withCount(['catalogs', 'customFieldGroups'])
            ->orderByDesc('updated_at')
            ->get()
            ->map(fn (CatalogType $type): object => (object) [
                'id' => $type->id,
                'name' => $type->name,
                'color' => $type->color,
                'field_count' => $type->customFields->count(),
                'group_count' => $type->custom_field_groups_count,
                'collection_count' => $type->catalogs_count,
    ...
    public function edit(Request $request, int $catalogType): View
    {
        $account = $request->user()->account;

        try {
            $type = $account->catalogTypes()
                ->with([
        ...
        return view('app.types.edit', [
            ...
            'catalogs' => $type->catalogs->sortBy('name'),
        ]);
    }

    public function update(Request $request, int $catalogType): RedirectResponse
    {
        $account = $request->user()->account;

        try {
            $type = $account->catalogTypes()->findOrFail($catalogType);
        } catch (ModelNotFoundException) {
            abort(404);
        }
    ...
    public function destroy(Request $request, int $catalogType): RedirectResponse
    {
        $account = $request->user()->account;
    ...
    private function fieldSummary(CatalogType $type): string
    {
